    ...
    public class MovieService
    {
        ...
        public List<MovieViewModel> GetMoviesByFilters(FilterType filterType, List<string> filters, bool isAndOperator)
        {
            ...
            try
            {
                ...
                if (filterType != FilterType.Directors && filterType != FilterType.Years && filterType != FilterType.Studio)
                {
                ...
                using (var context = new DatabaseContext())
                {
                    switch (filterType)
                    {
                        ...
                        case FilterType.Studio:
                            movies = context.Movies.Where(x => filters.Contains(x.Studio)).ToList();
                            break;
